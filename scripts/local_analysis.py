...
def load_sector_summary(sector_code=None, center=None):
    ...
    if not sector_code and center:
        best_dist = float("inf")
        for c in centroids:
            d = haversine_m(center[0], center[1], c["lat"], c["lng"])
            if d < best_dist:
                best_dist = d
                sector_code = c["sc"]
...
def identify_candidates(center, radius_km, data, osm_features, max_candidates=4):
    """Find nearby POIs around the given coordinates that could host a parcel locker.

    Source-agnostic: the center coordinates can come from MCLP, supermarket
    top-up, or direct user input. The search looks outward from the center
    for suitable placement sites (shops, supermarkets, transit hubs, etc.).
    """
    ...
    # Search for nearby POIs that could host a locker
    ...
    # Source 3: Transit hubs (stations are often good locker locations)
    ...
    print(f"  Raw candidate sites found: {len(candidates)}")
...
def score_candidates(candidates, osm_features, data, center=None):
    ...
    for i, cand in enumerate(candidates):
        ...
        # dist_m = distance from input target coordinates to the POI
        def _poi_dist_from_target(poi):
            if center:
                return round(haversine_m(center[0], center[1], poi["lat"], poi["lng"]))
            return poi["dist_m"]
        ...
        # Strip internal lat/lng from POI list before storing
        for poi in all_pois:
            poi.pop("lat", None)
            poi.pop("lng", None)
...
def generate_report(center, radius_km, travel_time, candidates, zoning, data,
    ...
    avg_score = round(sum(c["site_score"] for c in candidates) / max(1, len(candidates)), 1)
    ...
    if top:
        ...
        top_summary = (f"#{top['id']} {top.get('sector', '?')}{site_hint} "
                       f"(Score: {top['site_score']}) - {top['source']}")
    ...
    print(f"Avg site score: {avg_score}")
    if top:
...
def main():
    ...
    # Step 4: Score all candidates (cheap, local)
    ...
    if args.enrich:
        ...
        # 3. Within same verdict tier, sort by site_score descending
